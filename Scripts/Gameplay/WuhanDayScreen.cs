...
public partial class WuhanDayScreen : Control
{
    ...
    public override void _Ready()
    {
        ...
        CityDialogChrome.ApplyConfirmation(_abandon, StableIds.Cities.Wuhan, compact: true);
        ...
        _abandon.Confirmed += () => { Workstation.CancelAnimations(); _controller.AbandonDay(); AbandonRequested?.Invoke(); };
        VisibilityChanged += () =>
        {
    ...
    public bool Initialize(DataCatalog catalog, SaveService save, DayController controller, int day)
    {
        ...
        if (!controller.TryPrepareDay(StableIds.Cities.Wuhan,day,catalog,save.Data.Wuhan,out string error) || !save.ApplyStartUnlocks(controller.CurrentConfig!,out error)) { Feedback(error,true); return false; }
