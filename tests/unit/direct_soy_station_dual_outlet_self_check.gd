...
func _run() -> void:
	...
	if session != null:
		...
		_check(locked_station.cup_stack.visible and locked_station.cup_stack.texture_normal != null and locked_station.cup_stack.texture_normal.resource_path.ends_with("soy_milk_plastic_cup_stack_4_v3_bold_cartoon_transparent.png"), "workshop preview uses the stable representative soy-cup stack")
		...
		_check(live_station.cup_stack.texture_normal.resource_path.ends_with("soy_milk_plastic_cup_stack_4_v3_bold_cartoon_transparent.png") and (live_station.cup_stack.get_node("InventoryCountBadge") as Label).text == "×7", "cup stack artwork stays stable while the badge carries the real count")
		...
		_check((live_station.cup_stack.get_node("InventoryCountBadge") as Label).visible and (live_station.cup_stack.get_node("InventoryCountBadge") as Label).text == "×0", "empty cup stack keeps an explicit zero-count badge")
