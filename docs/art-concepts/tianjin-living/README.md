...
- 独立挂件与工具的编辑参考：`resource/art/TianJin/天津-煎饼-炸锅-豆浆-v2.png`，1672×941；三张动态工作台干净底图现以对应的 `*-v3.png` 为源，1672×941。
...
- 三阶段分别从各自 v3 原图复制，**只转移两个已授权局部**：源坐标挂件区域 `(1213,65,141,273)`，工具区域 `(1115,503,371,71)`；边缘 4 px 混合。区域外保留 v3 原图像素，包括左移后的炸锅与新增煎饼包装袋。原背景没有覆盖。
