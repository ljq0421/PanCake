...
const OUTPUT_DIR := "res://tmp/validation/customer_08_states_v4_colorlocked_gpu"
const STATES := [&"neutral", &"impatient", &"satisfied", &"accepting_bag", &"paying_coins"]
const EXPECTED_FILES := {
	&"neutral": "customer_08_neutral_v4_chinese.png",
	&"impatient": "customer_08_impatient_v4_colorlocked.png",
	&"satisfied": "customer_08_satisfied_v4_colorlocked.png",
	&"accepting_bag": "customer_08_accepting_bag_v4_colorlocked.png",
	&"paying_coins": "customer_08_paying_coins_v4_colorlocked.png",
}
...
func _run() -> void:
	...
	workstation.customer_queue.call("restore_active_customer", {}, &"customer_08")
	...
	for state in STATES:
		...
		for _frame in 3:
			await process_frame
		_check(portrait.texture != null, "%s state loads a customer_08 portrait" % state)
		if portrait.texture != null:
			var atlas := portrait.texture.get("atlas") as Texture2D
			_check(atlas != null and atlas.resource_path.ends_with(String(EXPECTED_FILES[state])), "%s state resolves its expected texture" % state)
		await RenderingServer.frame_post_draw
		var path := ProjectSettings.globalize_path("%s/customer_08_%s_v4_colorlocked_1920x1080.png" % [OUTPUT_DIR, state])
...
func _finish() -> void:
	if _failures.is_empty():
		print("CUSTOMER_08_STATES_GPU_PREVIEW_PASS")
		quit(0)
		return
	printerr("CUSTOMER_08_STATES_GPU_PREVIEW_FAIL\n" + "\n".join(_failures))
	quit(1)
