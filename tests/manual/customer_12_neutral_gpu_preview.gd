...
const SCREENSHOT_PATH := "res://tmp/validation/customer_12_neutral_v1_chinese_gpu_1920x1080.png"
...
func _run() -> void:
	...
	workstation.customer_queue.call("restore_active_customer", {}, &"customer_12")
	...
	_check(portrait != null and portrait.texture != null, "customer_12 neutral portrait loads in the real workstation")
	if portrait != null and portrait.texture != null:
		_check(portrait.texture.resource_path.ends_with("customer_12_neutral_cropped.tres"), "runtime uses the customer_12 neutral AtlasTexture")
		var atlas := portrait.texture.get("atlas") as Texture2D
		_check(atlas != null and atlas.resource_path.ends_with("customer_12_neutral_v1.png"), "AtlasTexture resolves the customer_12 neutral PNG")
		_check(portrait.texture.get("region") == Rect2(474, 26, 577, 998), "customer_12 crop contains full hair, hands, and below-waist anchor")
...
func _finish() -> void:
	if _failures.is_empty():
		print("CUSTOMER_12_NEUTRAL_GPU_PREVIEW_PASS")
		quit(0)
		return
	printerr("CUSTOMER_12_NEUTRAL_GPU_PREVIEW_FAIL\n" + "\n".join(_failures))
	quit(1)
