...
const SCREENSHOT_PATH := "res://tmp/validation/five_area_growth_ui_gpu_1920x1080.png"
...
func _run() -> void:
	...
	_check(ticket_1.disabled and ticket_2.disabled and ticket_3.disabled, "GPU day-end scene keeps D2 and D3 previews disabled on D1")
	var output_absolute := ProjectSettings.globalize_path(SCREENSHOT_PATH)
	var image := root.get_texture().get_image()
	var save_error := image.save_png(output_absolute)
	_check(save_error == OK and image.get_size() == Vector2i(1920, 1080), "GPU day-end scene captures at 1920x1080")
	station.queue_free()
	await process_frame
