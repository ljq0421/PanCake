...
func _initialize() -> void:
	...
	_check(bool(soy.call("take_empty_cup").get("success", false)), "soy serving starts by taking an empty cup")
	_check(bool(soy.call("fill_held_cup", 0.8).get("is_full", false)), "soy serving fills a cup after the 0.8-second hold")
	_check(bool(soy.call("add_sugar").get("success", false)), "soy serving can add one sugar portion")
	var soy_restored: RefCounted = SOY.new()
	soy_restored.call("load_snapshot", soy.call("snapshot"))
	var restored_soy := Dictionary(soy_restored.call("snapshot"))
	var restored_cup := Dictionary(restored_soy.get("cup", {}))
	_check(int(restored_cup.get("sugar_servings", 0)) == 1, "soy cup sweetness survives snapshot restore")
