...
public partial class StageOneSelfTest : Node
{
    ...
    private void RunCatalogTests()
    {
        ...
        Check(catalog.RecipesById.Keys.Count(id => id.StartsWith("pancake_", StringComparison.Ordinal)) == 8 && catalog.RecipesById.Keys.Count(id => id.StartsWith("hot_dry_noodles_", StringComparison.Ordinal)) == 6, "加载天津 8 个与武汉 6 个配方", $"实际 {catalog.RecipesById.Count}");
        ...
        Check(new[] { "normal", "office_worker", "regular", "big_order" }.All(catalog.CustomersById.ContainsKey) && catalog.CustomersById.Keys.Count(id => id.StartsWith("wuhan_", StringComparison.Ordinal)) == 5, "加载天津 4 类与武汉 5 类顾客", $"实际 {catalog.CustomersById.Count}");
