...
P1 第五名顾客的不耐烦半身 `Sprite2D` 状态。以已确认的 customer_05 中性绿色键控源为唯一身份和构图基准，只改变眉眼与嘴型。
...
```text
Use case: precise-object-edit. Edit the supplied customer_05 neutral chroma-key sprite into the IMPATIENT expression state.

Change ONLY the facial expression:
- eyebrows angle slightly downward toward the center,
- eyes mildly narrowed,
- mouth becomes a small restrained downward curve,
- emotion reads as mildly impatient, not angry or hostile.
...
Do not add or change posture, clothing, accessories, props, order card, patience bar, payment object, text, numbers, logos, watermark, comic symbols, motion marks, sweat drops, or background projection.
```
