...
const OUTPUT_DIR := "res://tmp/validation/customer_17_states_v1_gpu"
...
func _run() -> void:
	...
	for state in STATES:
		...
		for _frame in 3:
			await process_frame
		_check(portrait.texture != null and portrait.texture.resource_path.ends_with("customer_17_%s_cropped.tres" % state), "%s loads customer_17 AtlasTexture" % state)
		...
		var path := ProjectSettings.globalize_path("%s/customer_17_%s_v1_1920x1080.png" % [OUTPUT_DIR, state])
...
func _finish() -> void:
	if _failures.is_empty():
		print("CUSTOMER_17_STATES_GPU_PREVIEW_PASS")
		quit(0)
		return
	printerr("CUSTOMER_17_STATES_GPU_PREVIEW_FAIL\\n" + "\\n".join(_failures))
	quit(1)
