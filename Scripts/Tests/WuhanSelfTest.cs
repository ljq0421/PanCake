...
public partial class WuhanSelfTest : Node
{
    ...
    {
        var stock=new DoupiInventory();var machine=new DoupiStateMachine(c.DoupiGriddlesByLevel[1]);MakeBatch(machine,stock);Check(stock.Count==8,"豆皮一锅固定八块");MakeBatch(machine,stock);Check(stock.Count==16,"豆皮备餐盘容量十六块");var third=new DoupiStateMachine(c.DoupiGriddlesByLevel[1]);third.TryPourBatter();third.TryAddEgg();third.Tick(2.5);third.TryFlip();third.TryAddFilling();third.Tick(3.5);for(int i=0;i<4;i++)third.TryCut();Check(!third.TryStock(stock)&&third.State==DoupiState.Cut,"库存满时成品留在锅中等待");
    }
    private static void MakeBatch(DoupiStateMachine m,DoupiInventory s){m.TryPourBatter();m.TryAddEgg();m.Tick(2.5);m.TryFlip();m.TryAddFilling();m.Tick(3.5);for(int i=0;i<4;i++)m.TryCut();m.TryStock(s);}
    ...
    {
        string path=$"user://wuhan-v3-{Guid.NewGuid():N}.json";var save=new SaveService();AddChild(save);save.UsePathForTests(path);save.Data.Coins=3000;CityProgressData city=save.Data.Wuhan;city.UnlockedContentIds.Add("equipment:wuhan_ingredient_station_lv2");Check(save.TryPurchase(StableIds.Cities.Wuhan,"equipment:wuhan_ingredient_station_lv2",c,out _)&&save.Data.Coins==2880&&save.Data.PurchasedIngredientStationLevel==1,"武汉升级扣全局金币且不改变天津设备");DayConfig day12=c.GetDays(StableIds.Cities.Wuhan)[12];DayResult result=new(){Day=12,PlannedCustomers=26,CompletedCustomers=24,Satisfaction=90,PerfectOrders=15,SaleRevenue=411};DayPlan plan=new OrderGenerator().Generate(day12,c.RecipesById,c.ProductsById,c.CustomersById);save.CommitDay(result,plan,day12);Check(save.Data.Wuhan.Completed&&save.Data.Wuhan.BestStars==3&&save.Data.Wuhan.UnlockedCollectibleIds.Count==4,"武汉三星完成并保存收藏徽章");save.QueueFree();string absolute=ProjectSettings.GlobalizePath(path);if(File.Exists(absolute))File.Delete(absolute);
    }
    private void TestV2Migration()
    {
        string current=$"user://wuhan-migrate-v3-{Guid.NewGuid():N}.json",legacy=$"user://wuhan-migrate-v2-{Guid.NewGuid():N}.json";string legacyAbsolute=ProjectSettings.GlobalizePath(legacy);Directory.CreateDirectory(Path.GetDirectoryName(legacyAbsolute)!);
