...
public partial class PancakeWorkstation : Control
{
    ...
    [Export] public bool IsTianjinWorkbench { get; set; }
    [Export] public bool StoreFinishedPancakes { get; set; }
    private bool UseServingTray => StoreFinishedPancakes;
    private bool ProductionShortcutsEnabled => IsTianjinWorkbench;
    ...
    internal bool TryInvokeProductionShortcut(Key key)
    {
        ...
        if (key == Key.F && !IsFlipping)
        {
            if (Machine.Runtime.State is PancakeState.SideAReady or PancakeState.SideAOverdone)
            {
                ...
                _canvas.FlipEdge = 1;
                return Execute(PancakeCommand.Flip);
            }
            if (Machine.Runtime.State == PancakeState.Folded)
            {
                CancelFold();
                _directGeneration = Machine.Runtime.Generation;
                _directBag!.Begin(FoodCenter, Machine.Runtime.Quality == PancakeQuality.Overdone ? new Color(.82f, .56f, .33f) : Colors.White);
                _canvas.DirectFoodHidden = true;
                _directBag.Release(true, ReducedMotion);
                CompleteDirectBag();
                return true;
            }
        }

        if (key == Key.F && CanFoldGesture)
        {
            CancelFold();
            return Execute(PancakeCommand.Fold);
        }
    ...
    {
        "finished_pancake" => HasFinishedPancake && !IsTransferringBag && _directBag?.Animating != true,
    ...
    public override void _Ready()
    {
        ...
        _discardBatch.Pressed += () => { if (!IsTianjinWorkbench) ExecuteFryer(FryerCommand.Discard); };
        ...
        _discard.Pressed += () => { if (!IsTianjinWorkbench) Discard(); };
        ...
        if (IsTianjinWorkbench) ConfigureTianjinPresentation();
    }
    ...
    public void Tick(double deltaSeconds)
    {
        ...
        if (_trashSourceValid is not null && !_trashSourceValid()) _drag.CancelDrag();
        RenderLive();
    }
    ...
    private bool CanUse(string id) => _initialized && CanInteract && !IsFlipping && !_foldHeld && !DirectBusy && _enabledIngredients.Contains(id) && Inventory.HasAvailable(id);
    private bool CanDrop(string id) => CanInteract && !IsFlipping && !_foldHeld && !DirectBusy && id switch
    {
    ...
    {
        if (!_initialized || !CanInteract || IsFlipping || IsDirectDragging || (_directBag?.Animating == true && command != PancakeCommand.Bag) || _foldHeld) return false;
        if (command != PancakeCommand.Fold) CancelFold();
    ...
    private void Render()
    {
        ...
        if (IsTianjinWorkbench) _pancakeActions.Size = new Vector2(143, 48);
        if (IsTianjinWorkbench)
        {
            Rect2 footer = _pancakeActions.Visible ? new Rect2(598, 948, 300, 48) : new Rect2(598, 948, 455, 48);
        ...
        if (FryerMachine is not null)
        {
            ...
            _fryerStatus.Modulate = fryer.State == FryerState.Burnt ? TianjinUi.Red
                ...
                : TianjinUi.BrownText;
            _fryerStock.Text = IsTianjinWorkbench ? string.Empty : FryerMachine.Inventory.Count.ToString();
            if (IsTianjinWorkbench)
            {
        ...
        if (SoyMilkTray is not null)
        {
            ...
            _soyStatus.Text = IsTianjinWorkbench
    ...
    private void RenderLive()
    {
        ...
        if (IsTianjinWorkbench && Machine.Runtime.State == PancakeState.Burnt) _state.Text = "已经焦糊 · 长按右键拖到垃圾桶丢弃";
        if (IsTianjinWorkbench && FryerMachine?.Runtime.State == FryerState.Burnt) _fryerStatus.Text = "已经焦糊 · 长按右键拖到垃圾桶丢弃";
        _canvas.QueueRedraw();
        _stroke.RefreshVisualState();
        RenderTutorial();
    }
    ...
    {
        FryerState.Empty => IsTianjinWorkbench ? "长按炸锅 · 连续装料" : $"空篮 · 0/{machine.Level.Capacity} · 点击面坯装料",
        FryerState.Loaded => IsTianjinWorkbench ? "待下锅" : $"待下锅 · {machine.Runtime.Quantity}/{machine.Level.Capacity}",
        ...
        IReadOnlySet<string>? requiredToppings)
    {
        ...
        if (IsTianjinWorkbench && id == StableIds.Ingredients.Sauce && pancakeState is PancakeState.SideBCooking or PancakeState.SideBReady or PancakeState.Saucing)
