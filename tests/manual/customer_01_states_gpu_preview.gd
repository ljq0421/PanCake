...
const OUTPUT_DIR := "res://tmp/validation/customer_01_states_v6_palette_locked_gpu"
...
func _run() -> void:
	...
	for state in STATES:
		...
		var path := ProjectSettings.globalize_path("%s/customer_01_%s_v6_palette_locked_1920x1080.png" % [OUTPUT_DIR, state])
...
func _finish() -> void:
	if _failures.is_empty():
		print("CUSTOMER_01_STATES_GPU_PREVIEW_PASS")
		quit(0)
		return
	printerr("CUSTOMER_01_STATES_GPU_PREVIEW_FAIL\n" + "\n".join(_failures))
	quit(1)
