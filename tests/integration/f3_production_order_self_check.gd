...
func _run() -> void:
	...
	_check(bool(stored_youtiao.get("success", false)) and bool(delivered_youtiao.get("success", false)) and int(Dictionary(session.call("f3_machine_snapshot", &"device.youtiao_fryer")).get("quantity", -1)) == 0 and int(Dictionary(session.call("prepared_product_slot_status", &"slot.04")).get("count", 0)) == 1, "the whole fryer batch enters prepared storage before one youtiao is staged to the order")
