...
卡面、悬停和按钮禁用状态必须由同一个展示状态生成。普通可购买项三者统一为“可预订，明日生效”；内部金币保底项仍按金币 `x/y` 展示，但玩家侧只能看到普通“安装位/内容位”和金币进度，不得出现“金币保底”字样或解释其替代了哪些成长条件。其他不可购买项的卡面取 `missing_requirements` 的第一项，悬停列出全部缺口。所有数值门槛使用实际进度 `x/y`；未知 ID 或未知原因显示“成长配置异常，无法预订”并记录错误，不允许回退为“暂不满足条件”。
...
保护性教学单额外要求当前库存至少能制作一份；不足时教学留在队列中，营业前 UI 显示缺少的补货，不生成一张必定失败的教学单。

### 11.3 教学优先级

1. 若已有 `active_tutorial_area_id`，且当天尚未生成教学单，尝试生成该区域教学单；成品饮品教学固定生成一份常温纯牛奶订单，即使纯牛奶库存为 0 也先展示顾客并提示补货，且不赠送免费库存；
