...
public partial class InterfaceTeachingSelfTest : Node
{
    private static string Output => OS.GetCmdlineUserArgs().Contains("--teaching-layout")
        ? "res://artifacts/teaching-layout-20260917/interface" : "res://artifacts/hover-removal-20260917";
    ...
    public override async void _Ready()
    {
        try
        {
            ...
            _save.UsePathForTests(Output + "/fixture.json"); Check(_save.ResetProgress(out _), "isolated save");
            ...
            await NoHover(Find<Button>(start, "WallMap"));
            ...
            guide = Guide(book); Check(guide?.LessonKey == InterfaceLessons.BookKey, "book first-use teaching");
            ...
            Check(Find<Button>(start, "ReplayInterfaceTeaching").IsVisibleInTree(), "help exposes replay entry"); await Shot("11-help-entry");
            Click(Find<Button>(start, "ReplayInterfaceTeaching")); await Frames(); guide = Guide(start);
            Check(guide is not null && guide.StepCount == 11, "help replays all approved topics");
            _viewport.Size = new(1280, 720); await Frames(10); await Shot("12-help-replay-1280");
            Key(Godot.Key.Escape); await Frames(); Check(start.ModalOpen && Guide(start) is null, "Esc returns from replay to help");
            ...
            foreach (var lesson in InterfaceLessons.Replay(StableIds.Cities.Tianjin))
            {
                ...
                if (guide.StepIndex == 9) await Shot("13-help-replay-en");
                Click(Find<Button>(guide, "NextTeaching")); await Frames();
            }
