...
const SCREENSHOT_PATH := "res://tmp/validation/customer_05_neutral_v4_chinese_gpu_1920x1080.png"
...
func _run() -> void:
	...
	_check(portrait != null and portrait.texture != null, "customer_05 neutral portrait loads in the real workstation")
	if portrait != null and portrait.texture != null:
		_check(portrait.texture.resource_path.ends_with("customer_05_neutral_cropped.tres"), "runtime uses the customer_05 neutral AtlasTexture")
		var atlas := portrait.texture.get("atlas") as Texture2D
		_check(atlas != null and atlas.resource_path.ends_with("customer_05_neutral_v4_chinese.png"), "AtlasTexture resolves the new v4 neutral PNG")
		_check(portrait.texture.get("region") == Rect2(536, 86, 451, 891), "AtlasTexture preserves the old crop and anchor contract")
...
func _finish() -> void:
	if _failures.is_empty():
		print("CUSTOMER_05_NEUTRAL_GPU_PREVIEW_PASS")
		quit(0)
		return
	printerr("CUSTOMER_05_NEUTRAL_GPU_PREVIEW_FAIL\n" + "\n".join(_failures))
	quit(1)
