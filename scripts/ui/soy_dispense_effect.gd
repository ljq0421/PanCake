...
func play_sugar_add(source: Vector2) -> void:
	# A larger, short ingredient trail makes the otherwise instant sweetness
...
func _draw_sugar_add() -> void:
	...
	if _reduced_motion:
		...
		for index in range(4):
			...
			draw_circle(Vector2(x, target.y - 2.0), 4.5, Color("fff2be"))
		return
