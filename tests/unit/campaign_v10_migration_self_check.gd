...
func _run() -> void:
	...
	_check(migrated.has_save() and int(Dictionary(migrated.get("_campaign_data")).get("version", 0)) == 13, "legacy save migrates to cartoon-breakfast campaign version thirteen")
	...
	_check(int(Dictionary(breakfast.get("progression", {})).get("coins", -1)) == 0 and int(Dictionary(breakfast.get("progression", {})).get("current_day", -1)) == 1, "legacy breakfast progress resets to day one")
	_check(is_equal_approx(float(breakfast.get("business_day_remaining_seconds", 0.0)), migrated.FIRST_BUSINESS_DAY_DURATION_SECONDS), "legacy breakfast timer resets to the first-day duration")
	_check(Array(Dictionary(breakfast.get("formal_orders", {})).get("active_order_ids", [])).is_empty(), "legacy active breakfast orders are discarded")
