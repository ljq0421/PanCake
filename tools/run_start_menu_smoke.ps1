...
$logFile = Join-Path $env:TEMP ("projectcake-start-menu-smoke-{0}.log" -f [Guid]::NewGuid().ToString('N'))
& $godot --path $project --log-file $logFile -s res://tests/integration/start_menu_visual_smoke.gd
exit $LASTEXITCODE
