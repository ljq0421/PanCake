...
func _check_scene_contract() -> void:
	...
	var youtiao_locked_click_layer := workstation.get_node("SafeArea/FiveAreaStationClickLayers/YoutiaoLockedClickLayer") as Button
	...
	var chili_sauce_refill_button := workstation.get_node("SafeArea/RightRack/ChiliSauceRefillButton") as Button
	...
	_check(left_panel_style is StyleBoxEmpty or (left_panel_style is StyleBoxFlat and (left_panel_style as StyleBoxFlat).bg_color.a <= 0.01), "left tool rack no longer draws a floating panel")
	_check(ingredient_panel_style is StyleBoxEmpty or (ingredient_panel_style is StyleBoxFlat and (ingredient_panel_style as StyleBoxFlat).bg_color.a <= 0.01), "right ingredient rack no longer draws a floating panel")
	...
	_check(fold_button.size == Vector2(122, 76) and fold_button.toggle_mode, "fold tool keeps its state-driven five-area hit region")
	_check(not sauce_brush_button.visible and sauce_brush_button.disabled and sauce_brush_button.mouse_filter == Control.MOUSE_FILTER_IGNORE, "standalone sauce brush has left the player interaction flow")
	_check(ladle_button.get_global_rect().end.x <= pan_base.get_global_rect().position.x and spreader_button.global_position.x >= pan_base.get_global_rect().end.x and fold_button.global_position.x >= pan_base.get_global_rect().end.x, "the active pancake tools flank rather than cover the central griddle")
