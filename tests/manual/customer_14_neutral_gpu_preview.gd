...
const OUTPUT_PATH := "res://tmp/validation/customer_14_neutral_v3_chinese_gpu_1920x1080.png"
const EXPECTED_REGION := Rect2(532, 92, 465, 869)
...
func _run() -> void:
	...
	# The tutorial can leave its order card open over the left side of the scene.
	# This preview hides only that overlay, not the gameplay portrait.
	...
	for _frame in 3:
		await process_frame
	print("CUSTOMER_14_PREVIEW_PORTRAIT visible=%s position=%s texture=%s" % [portrait.visible, portrait.global_position, portrait.texture.resource_path if portrait.texture != null else "<null>"])
	_check(portrait.texture != null and portrait.texture.resource_path.ends_with("customer_14_neutral_cropped.tres"), "customer_14 neutral portrait loads in the real workstation")
	if portrait.texture != null:
		var atlas := portrait.texture.get("atlas") as Texture2D
		_check(atlas != null and atlas.resource_path.ends_with("customer_14_neutral_v3_keyclean120.png"), "AtlasTexture resolves the customer_14 neutral PNG")
		_check(portrait.texture.get("region") == EXPECTED_REGION, "customer_14 crop contains full hair, hands, and below-waist anchor")
...
func _finish() -> void:
	if _failures.is_empty():
		print("CUSTOMER_14_NEUTRAL_GPU_PREVIEW_PASS")
		quit(0)
		return
	printerr("CUSTOMER_14_NEUTRAL_GPU_PREVIEW_FAIL\n" + "\n".join(_failures))
	quit(1)
