...
func _run() -> void:
	...
	if session != null:
		...
		_check(StringName(remaining_right.get("cup_state", &"")) == &"held_empty" and bool(remaining_right.get("secondary_empty_cup_placed", false)) and remaining_right_station.nozzle_button.disabled and not remaining_right_station.second_nozzle_button.disabled and remaining_right_station.machine_output.position.is_equal_approx(remaining_right_station._secondary_cup_position()), "after the left cup leaves, the remaining right empty cup stays at and is operated from the right outlet")
