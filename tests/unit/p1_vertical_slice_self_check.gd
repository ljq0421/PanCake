...
func _test_customer_queue_rotation() -> void:
	...
	_check(
		rotated_ids == PackedStringArray(["customer_01", "customer_02", "customer_03", "customer_04", "customer_05", "customer_06", "customer_07", "customer_08", "customer_09", "customer_10"])
		and StringName(Dictionary(single_queue.call("current_customer")).get("id", &"")) == &"customer_01",
		"customer queue rotates through all ten enabled identities before wrapping"
	)
