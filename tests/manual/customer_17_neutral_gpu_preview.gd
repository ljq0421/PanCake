...
const OUTPUT_PATH := "res://tmp/validation/customer_17_neutral_v1_chinese_gpu_clear_1920x1080.png"
const EXPECTED_REGION := Rect2(535, 122, 459, 834)
...
func _run() -> void:
	...
	workstation.customer_queue.call("restore_active_customer", {}, &"customer_17")
	...
	for _frame in 3:
		await process_frame
	print("CUSTOMER_17_PREVIEW_PORTRAIT visible=%s position=%s texture=%s" % [portrait.visible, portrait.global_position, portrait.texture.resource_path if portrait.texture != null else "<null>"])
	_check(portrait.texture != null and portrait.texture.resource_path.ends_with("customer_17_neutral_cropped.tres"), "customer_17 neutral portrait loads in the real workstation")
	if portrait.texture != null:
		var atlas := portrait.texture.get("atlas") as Texture2D
		_check(atlas != null and atlas.resource_path.ends_with("customer_17_neutral_v1_keyclean.png"), "AtlasTexture resolves the selected customer_17 neutral PNG")
		_check(portrait.texture.get("region") == EXPECTED_REGION, "customer_17 crop contains full hair, hands, and below-waist anchor")
...
func _finish() -> void:
	if _failures.is_empty():
		print("CUSTOMER_17_NEUTRAL_GPU_PREVIEW_PASS")
		quit(0)
		return
	printerr("CUSTOMER_17_NEUTRAL_GPU_PREVIEW_FAIL\\n" + "\\n".join(_failures))
	quit(1)
