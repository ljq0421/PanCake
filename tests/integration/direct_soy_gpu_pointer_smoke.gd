...
func _run() -> void:
	...
	_check(StringName(Dictionary(session.call("f3_machine_snapshot", &"device.fresh_soy_milk_machine")).get("cup", {}).get("temperature_mode", &"")) == &"iced", "mouse click adds ice")
	_check(float(soy_station.dispense_effect.get("_ice_animation_time")) >= 0.0, "adding ice starts the visible tray-to-cup action")
...
func _setup_soy_session(session: Node) -> void:
	...
	progression.set("owned_assist_ids", {&"assist.fresh_soy_milk.sugar": true, &"assist.fresh_soy_milk.ice": true})
