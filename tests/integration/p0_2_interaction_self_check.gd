...
func _run() -> void:
	...
	_move_surface(surface, pan_center + Vector2(80, 0))
	workstation._process(1.0 / 60.0)
	_release_surface(surface, surface.pointer_local_position)
	...
	_check(workstation.tool_controller.current_tool == ToolController.Tool.NONE and not surface.cursor_is_t_spreader, "implicit spread confirmation keeps the batter spreader returned")

	_cancel_surface(surface, surface.pointer_local_position)
	_check(workstation.tool_controller.current_tool == ToolController.Tool.NONE, "right mouse remains safe after implicit tool return")
