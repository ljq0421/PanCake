...
const OUTPUT_DIR := "res://tmp/validation/customer_09_states_v4_gpu"
const STATES := [&"impatient", &"satisfied", &"accepting_bag", &"paying_coins"]
const EXPECTED_REGIONS := [
	Rect2(541, 76, 438, 883),
	Rect2(541, 76, 438, 883),
	Rect2(528, 60, 457, 947),
	Rect2(450, 67, 640, 910),
]
...
func _run() -> void:
	...
	for index in STATES.size():
		...
		if portrait.texture != null:
			_check(portrait.texture.resource_path.ends_with("customer_09_%s_cropped.tres" % state), "%s uses the existing customer_09 AtlasTexture" % state)
			var atlas := portrait.texture.get("atlas") as Texture2D
			_check(atlas != null and atlas.resource_path.ends_with("customer_09_%s_v4_chinese.png" % state), "%s AtlasTexture resolves the new v4 PNG" % state)
			_check(portrait.texture.get("region") == EXPECTED_REGIONS[index], "%s preserves the old crop and anchor contract" % state)
		await RenderingServer.frame_post_draw
		var path := ProjectSettings.globalize_path("%s/customer_09_%s_v4_chinese_1920x1080.png" % [OUTPUT_DIR, state])
...
func _finish() -> void:
	if _failures.is_empty():
		print("CUSTOMER_09_STATES_GPU_PREVIEW_PASS")
		quit(0)
		return
	printerr("CUSTOMER_09_STATES_GPU_PREVIEW_FAIL\n" + "\n".join(_failures))
	quit(1)
