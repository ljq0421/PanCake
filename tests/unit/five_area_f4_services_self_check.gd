...
func _initialize() -> void:
	...
	_check(StringName(soy.call("snapshot").get("state", &"")) == &"ready_safe", "base soy batch completes and waits for manual cups")
	_check(StringName(soy.call("collect", 1).get("reason", &"")) == &"cup_required", "soy output cannot bypass the cup action")
