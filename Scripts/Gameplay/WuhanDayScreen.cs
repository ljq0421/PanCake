...
public partial class WuhanDayScreen : Control
{
    ...
    private Button _bowlImage = null!, _doupiButton = null!, _eggButton = null!; private HBoxContainer _basketRow = null!, _ingredientRow = null!;
    private TextureRect _cookerImage = null!, _doupiImage = null!;
    ...
    private ConfirmationDialog _abandon = null!; private bool _committed; private bool _focused = true; private double _feedbackSeconds; private Vector2? _mixLast;
    ...
    public void ConnectController(DayController controller)
    {
        ...
        controller.DeliveryCompleted += result => Feedback(result.Message, result.Grade is DeliveryGrade.Incorrect or DeliveryGrade.Rejected);
    }
    public void Initialize(DataCatalog catalog, SaveService save, DayController controller, int day)
    {
        ...
        _basketButtons[1].Visible=_cooker.Baskets.Count>1; _doupiButton.Disabled=_doupi is null; _eggButton.Disabled=_egg is null; Render();
    }
    ...
    public override void _Process(double delta)
    {
        ...
        if (!_focused || !IsVisibleInTree() || _controller?.CurrentConfig is null) return;
        _controller.Tick(delta); _cooker?.Tick(delta); _doupi?.Tick(delta); _egg?.Tick(delta);
    ...
    {
        if (what==NotificationApplicationFocusOut) { _focused=false; if(_controller is not null)_controller.IsPaused=true; }
        if (what==NotificationApplicationFocusIn) { _focused=true; if(_controller is not null)_controller.IsPaused=false; }
    }
    ...
    private void Build()
    {
        ...
        _abandon=new ConfirmationDialog { Title="提前打烊？",DialogText="本日收入和成绩不会保存。",OkButtonText="打烊并返回" }; _abandon.Confirmed+=()=>{_controller.AbandonDay();HubRequested?.Invoke();}; AddChild(_abandon);
    }
    ...
    private void BasketAction(int index)
    {
        ...
        else if(item.State==NoodleBasketState.Drained){if(_cooker.TryTake(index,out NoodleQuality q)&&_bowl.TryAddNoodles(q)){ok=true;message="熟面倒入碗中。";}else message="拌面碗还没有空出来。";}
        else message="面还在烫，等到最佳窗口再提篮。";
        if(!ok&&item.State==NoodleBasketState.Empty&&_ingredients.Count(StableIds.Ingredients.WuhanNoodles)==0)StartRefill(StableIds.Ingredients.WuhanNoodles);Feedback(message,!ok);
    }
    private void IngredientAction(string id)
    {
        ...
        if(ok){_ingredients.TryConsume(id);Feedback("配料已经加入。",false);}else if(_ingredients.Count(id)<=2){StartRefill(id);Feedback("当前步骤不能加料，已开始补充低库存。",false);}else Feedback("先把熟面和基础调味放进碗里。",true);
    }
    private void StartRefill(string id){if(!_refills.ContainsKey(id))_refills[id]=1.0;}
    private void OnMixInput(InputEvent input)
    {
        if(input is InputEventMouseButton mb){_mixLast=mb.ButtonIndex==MouseButton.Left&&mb.Pressed?mb.Position:null;}
        else if(input is InputEventMouseMotion mm && (mm.ButtonMask&MouseButtonMask.Left)!=0){if(_mixLast is Vector2 last)_bowl.AddMixDistance(last.DistanceTo(mm.Position));_mixLast=mm.Position;}
    }
    private void DeliverNoodles()
    {
        ...
        DeliveryEvaluation result=_controller.TryDeliverWuhanSelected(item,()=>{_bowl.Reset();return true;});Feedback(result.Message,result.Grade is DeliveryGrade.Rejected or DeliveryGrade.Incorrect);
    }
    private void DoupiAction()
    {
        ...
        Feedback(ok?"豆皮操作完成一步。":"豆皮正在煎制，请观察状态。",!ok);
    }
    private bool DiscardDoupi(){_doupi!.Discard();return true;}
    private void DeliverDoupi()
    {
        ...
        DeliveryEvaluation result=_controller.TryDeliverWuhanSelected(new DeliveredItem(ProductKind.Doupi,StableIds.Products.Doupi,null,null,null,flags),()=>_doupiStock.TryTake(1,out _));Feedback(result.Message,result.Grade is DeliveryGrade.Rejected or DeliveryGrade.Incorrect);
    }
    private void EggAction()
    {
        if(_egg is null){Feedback("蛋酒台将在 Day 6 解锁。",true);return;}
        if(_egg.HasFinishedCup){DeliveryEvaluation result=_controller.TryDeliverWuhanSelected(new DeliveredItem(ProductKind.EggRiceWine,StableIds.Products.EggRiceWine),_egg.TryTake);Feedback(result.Message,result.Grade is DeliveryGrade.Rejected or DeliveryGrade.Incorrect);return;}
        if(_egg.BaseCups==0){bool refill=_egg.TryRefill();Feedback(refill?"补充蛋酒底料，0.6 秒后完成。":"蛋酒台正在工作。",!refill);return;}
        bool started=_egg.TryStart();Feedback(started?"正在冲蛋酒，0.6 秒后再次点击交付。":"蛋酒台正在工作。",!started);
    }
    ...
    private void Render()
    {
        ...
        _bowlStatus.Text=$"{_bowl.State} · 拌匀 {_bowl.MixProgress:0}%";foreach(Node node in _ingredientRow.GetChildren()){var button=(Button)node;string id=button.GetMeta("ingredient_id").AsString();button.Text=$"{IngredientName(id)} {_ingredients.Count(id)}{(_refills.ContainsKey(id)?" 补料中":"")}";}
        _doupiStatus.Text=_doupi is null?"Day 4 解锁":$"{_doupi.State} · 已切 {_doupi.CompletedCuts}/4 · 库存 {_doupiStock.Count}/16";_doupiButton.Text=_doupi?.State switch{DoupiState.Empty=>"浇浆",DoupiState.Batter=>"加鸡蛋",DoupiState.ReadyToFlip=>"翻面",DoupiState.Flipped=>"铺三鲜糯米馅",DoupiState.ReadyToCut or DoupiState.Overbrowned=>"切块",DoupiState.Cut=>"收入备餐盘",DoupiState.Burnt=>"丢弃焦糊豆皮",_=>"煎制中"};
    ...
    {
        if(_committed||_controller.CurrentConfig?.CityId!=StableIds.Cities.Wuhan)return;_committed=true;try{DayCommitResult commit=_save.CommitDay(result,_controller.CurrentPlan!,_controller.CurrentConfig!);string stars=result.Day==12?$"\n武汉评级 {new string('★',commit.EarnedStars)}{new string('☆',3-commit.EarnedStars)}":"";_resultText.Text=$"[center][font_size=28]武汉 Day {result.Day} 打烊[/font_size]\n\n[font_size=42]今日总收入 ¥{result.TotalRevenue}[/font_size]\n永久金币增加 ¥{commit.PermanentCoinGain}\n\n完成 {result.CompletedCustomers} 位 · 流失 {result.LostCustomers} 位\n满意度 {result.Satisfaction:0}% · Perfect {result.PerfectOrders} 单{stars}[/center]";_unlock.Text=commit.NewChapterCompletion?"武汉 · 过早之城已经点亮！获得三件早餐收藏与章节徽章。下一站筹备中。":_controller.CurrentConfig.CompletionUnlocks.Count>0?"新的武汉设备升级已经开放。":"成绩已写入武汉经营手账。";}catch(IOException e){_resultText.Text=$"保存失败：{e.Message}";_unlock.Text="本次结果已回退。";}_blocker.Visible=true;_results.Visible=true;
    }
