...
const SCREENSHOT_PATH := "res://tmp/validation/customer_07_neutral_v4_chinese_gpu_1920x1080.png"
...
func _run() -> void:
	...
	_check(portrait != null and portrait.texture != null, "customer_07 neutral portrait loads in the real workstation")
	if portrait != null and portrait.texture != null:
		_check(portrait.texture.resource_path.ends_with("customer_07_neutral_cropped.tres"), "runtime uses the customer_07 neutral AtlasTexture")
		_check(portrait.texture.region == Rect2(489, 110, 541, 867), "AtlasTexture preserves the old crop and anchor contract")
		var atlas := portrait.texture.get("atlas") as Texture2D
		_check(atlas != null and atlas.resource_path.ends_with("customer_07_neutral_v4_chinese.png"), "AtlasTexture resolves the new v4 neutral PNG")
...
func _finish() -> void:
	if _failures.is_empty():
		print("CUSTOMER_07_NEUTRAL_GPU_PREVIEW_PASS")
		quit(0)
		return
	printerr("CUSTOMER_07_NEUTRAL_GPU_PREVIEW_FAIL\n" + "\n".join(_failures))
	quit(1)
