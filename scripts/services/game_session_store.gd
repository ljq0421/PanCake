...
const NIGHT_MARKET_CHAPTER_ID := &"chapter.night_market"
const BREAKFAST_SCENE_PATH := "res://scenes/main/main.tscn"
const NOODLE_SCENE_PATH := "res://scenes/main/noodle_shop_main.tscn"
const NIGHT_MARKET_SCENE_PATH := "res://scenes/main/night_market_main.tscn"
...
var _night_market_session: RefCounted
...
func _ready() -> void:
	...
	elif has_save() and _active_chapter == NIGHT_MARKET_CHAPTER_ID:
		_restore_night_market_session()
	else:
...
func chapter_scene_path(chapter_id: StringName) -> String:
	...
	if chapter_id == NIGHT_MARKET_CHAPTER_ID:
		return NIGHT_MARKET_SCENE_PATH
	return ""


func chapter_statuses() -> Array[Dictionary]:
	return [chapter_status(BREAKFAST_CHAPTER_ID), chapter_status(NOODLE_CHAPTER_ID), chapter_status(NIGHT_MARKET_CHAPTER_ID)]


func chapter_status(chapter_id: StringName) -> Dictionary:
	if chapter_id not in [BREAKFAST_CHAPTER_ID, NOODLE_CHAPTER_ID, NIGHT_MARKET_CHAPTER_ID]:
	...
	if chapter_id == NOODLE_CHAPTER_ID:
		progress = _breakfast_milestone_progress(_breakfast_chapter_snapshot())
	elif chapter_id == NIGHT_MARKET_CHAPTER_ID:
		progress = _noodle_milestone_progress(_noodle_chapter_snapshot())
	var day := 1
	var coins := 0
...
func select_chapter(chapter_id: StringName) -> Dictionary:
	...
	elif chapter_id == NIGHT_MARKET_CHAPTER_ID and not chapters.has(str(chapter_id)):
		_night_market_session = NIGHT_MARKET_SESSION.new()
		chapters[str(chapter_id)] = _night_market_session.call("snapshot")
...
func night_market_shop_snapshot() -> Dictionary:
	_ensure_night_market_session()
	return Dictionary(_night_market_session.call("snapshot")).duplicate(true)


func night_market_ensure_active_order() -> Dictionary:
	_ensure_night_market_session()
	var result := Dictionary(_night_market_session.call("ensure_active_order"))
	_sync_night_market_session_to_save()
	_touch_and_write()
	return result


func night_market_add_grill_skewer(item_id: StringName, zone_id: StringName) -> Dictionary:
	return _night_market_mutation(&"add_grill_skewer", [item_id, zone_id])


func night_market_flip_grill_slot(slot_index: int) -> Dictionary:
	return _night_market_mutation(&"flip_grill_slot", [slot_index])


func night_market_plate_grill_slot(slot_index: int) -> Dictionary:
	return _night_market_mutation(&"plate_grill_slot", [slot_index])


func night_market_add_fryer_item(item_id: StringName) -> Dictionary:
	return _night_market_mutation(&"add_fryer_item", [item_id])


func night_market_set_fryer_power(power_id: StringName) -> Dictionary:
	return _night_market_mutation(&"set_fryer_power", [power_id])


func night_market_lower_fryer() -> Dictionary:
	return _night_market_mutation(&"lower_fryer")


func night_market_lift_fryer() -> Dictionary:
	return _night_market_mutation(&"lift_fryer")


func night_market_plate_fryer() -> Dictionary:
	return _night_market_mutation(&"plate_fryer")


func night_market_season_last(seasoning_id: StringName) -> Dictionary:
	return _night_market_mutation(&"season_last", [seasoning_id])


func night_market_serve_plate() -> Dictionary:
	_ensure_night_market_session()
	var result := Dictionary(_night_market_session.call("serve_plate"))
	if bool(result.get("success", false)):
		_apply_global_reputation_delta(int(result.get("reputation_delta", 0)))
		_sync_night_market_session_to_save()
		_touch_and_write()
...
func night_market_collect_payment() -> Dictionary:
	_ensure_night_market_session()
	var result := Dictionary(_night_market_session.call("collect_payment"))
	if bool(result.get("success", false)):
		_sync_night_market_session_to_save()
		_touch_and_write()
		coins_changed.emit(int(result.get("coins", 0)))
...
func night_market_refuse_order() -> Dictionary:
	_ensure_night_market_session()
	var result := Dictionary(_night_market_session.call("refuse_order"))
	if bool(result.get("success", false)):
		_apply_global_reputation_delta(int(result.get("reputation_delta", 0)))
		_sync_night_market_session_to_save()
		_touch_and_write()
...
func night_market_discard_production() -> Dictionary:
	return _night_market_mutation(&"discard_production")


func night_market_advance(delta: float) -> Dictionary:
	_ensure_night_market_session()
	var result := Dictionary(_night_market_session.call("advance", delta))
	var reputation_delta := int(result.get("reputation_delta", 0))
	if reputation_delta != 0:
		_apply_global_reputation_delta(reputation_delta)
		campaign_changed.emit(campaign_snapshot())
...
func night_market_restock(stock_id: StringName) -> Dictionary:
	_ensure_night_market_session()
	var result := Dictionary(_night_market_session.call("restock", stock_id))
	if bool(result.get("success", false)):
		_sync_night_market_session_to_save()
		_touch_and_write()
		coins_changed.emit(int(result.get("coins", 0)))
...
func night_market_growth_overview() -> Array[Dictionary]:
	_ensure_night_market_session()
	return Array(_night_market_session.call("growth_overview")).duplicate(true)
...
func night_market_milestone_progress() -> Dictionary:
	_ensure_night_market_session()
	return Dictionary(_night_market_session.call("milestone_progress")).duplicate(true)


func night_market_end_day(reason: StringName = &"manual") -> Dictionary:
	_ensure_night_market_session()
...
func night_market_begin_next_day() -> Dictionary:
	_ensure_night_market_session()
	var result := Dictionary(_night_market_session.call("begin_next_day"))
	if bool(result.get("success", false)):
		_sync_night_market_session_to_save()
		_touch_and_write(true)
	return result
...
func _night_market_mutation(method_name: StringName, arguments: Array = []) -> Dictionary:
	_ensure_night_market_session()
	var result := Dictionary(_night_market_session.callv(method_name, arguments))
	if bool(result.get("success", false)):
		_sync_night_market_session_to_save()
		_touch_and_write()
	return result
...
func continue_game() -> bool:
	...
	elif _active_chapter == NIGHT_MARKET_CHAPTER_ID:
		_ensure_night_market_session()
		_night_market_session.call("set_business_paused", true)
		_save_data = Dictionary(_night_market_session.call("snapshot")).duplicate(true)
	else:
...
func is_business_paused() -> bool:
	...
	if _active_chapter == NIGHT_MARKET_CHAPTER_ID:
		_ensure_night_market_session()
		return bool(_night_market_session.get("business_paused"))
...
func set_business_paused(paused: bool) -> void:
	...
	if _active_chapter == NIGHT_MARKET_CHAPTER_ID:
		_ensure_night_market_session()
		_night_market_session.call("set_business_paused", paused)
		_sync_night_market_session_to_save()
		_touch_and_write()
		return
...
func _ensure_campaign_shape() -> void:
	...
	for value in Array(campaign.get("unlocked_chapter_ids", [])):
		...
		if chapter_id in [BREAKFAST_CHAPTER_ID, NOODLE_CHAPTER_ID, NIGHT_MARKET_CHAPTER_ID] and not unlocked.has(chapter_id):
...
func _load_active_chapter_snapshot() -> void:
	...
	if _active_chapter == BREAKFAST_CHAPTER_ID:
		_ensure_save_shape()
	elif _active_chapter == NOODLE_CHAPTER_ID:
		_restore_noodle_session()
	else:
		_restore_night_market_session()
...
func _noodle_chapter_snapshot() -> Dictionary:
	if _active_chapter == NOODLE_CHAPTER_ID:
		if _noodle_session != null:
			return Dictionary(_noodle_session.call("snapshot")).duplicate(true)
		if not _save_data.is_empty():
			return _save_data.duplicate(true)
	return Dictionary(Dictionary(_campaign_data.get("chapters", {})).get(str(NOODLE_CHAPTER_ID), {})).duplicate(true)


func _noodle_milestone_progress(noodle_snapshot: Dictionary) -> Dictionary:
	var owned_value: Variant = noodle_snapshot.get("owned_growth_ids", [])
	var owned_count := 0
	for growth_id in NOODLE_SHOP_CATALOG.GROWTH_DISPLAY_ORDER:
		var is_owned := false
		if owned_value is Dictionary:
			var owned := Dictionary(owned_value)
			is_owned = bool(owned.get(growth_id, owned.get(str(growth_id), false)))
		elif owned_value is Array or owned_value is PackedStringArray:
			is_owned = Array(owned_value).has(growth_id) or Array(owned_value).has(str(growth_id))
		if is_owned:
			owned_count += 1
	return {
		"owned_growth_count": owned_count,
		"required_growth_count": NOODLE_SHOP_CATALOG.GROWTH_DISPLAY_ORDER.size(),
		"complete": owned_count == NOODLE_SHOP_CATALOG.GROWTH_DISPLAY_ORDER.size(),
	}
...
func _refresh_campaign_unlocks() -> bool:
	...
	if not unlocked.has(NOODLE_CHAPTER_ID) and bool(_breakfast_milestone_progress(_breakfast_chapter_snapshot()).get("complete", false)):
		unlocked.append(NOODLE_CHAPTER_ID)
		changed = true
	if not unlocked.has(NIGHT_MARKET_CHAPTER_ID) and bool(_noodle_milestone_progress(_noodle_chapter_snapshot()).get("complete", false)):
		unlocked.append(NIGHT_MARKET_CHAPTER_ID)
		changed = true
	campaign["unlocked_chapter_ids"] = unlocked
	_campaign_data["campaign"] = campaign
	return changed
...
func _restore_night_market_session() -> void:
	_night_market_session = NIGHT_MARKET_SESSION.new(_save_data)


func _ensure_night_market_session() -> void:
	if _night_market_session == null:
		_restore_night_market_session()
...
func _sync_night_market_session_to_save() -> void:
	if _active_chapter != NIGHT_MARKET_CHAPTER_ID:
		return
	_ensure_night_market_session()
	_save_data = Dictionary(_night_market_session.call("snapshot")).duplicate(true)
...
func _active_chapter_day_open() -> bool:
	...
	if _active_chapter == NIGHT_MARKET_CHAPTER_ID:
		_ensure_night_market_session()
		return bool(_night_market_session.get("day_open"))
