...
func _run() -> void:
	...
	_check(bool(disabled_auto.get("success", false)) and not bool(session.call("youtiao_auto_lift_enabled")) and StringName(Dictionary(session.call("f3_machine_snapshot", &"device.youtiao_fryer")).get("state", &"")) == &"ready_safe", "disabled auto-lift preserves the manual lift window")
