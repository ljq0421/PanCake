"""66-second Chinese revision: Tianjin departure, earnings, unlocks and collection."""
...
def main():
    ...
    cuts = [
        ('tianjin', 'spread', 0, 1.4, 1.4, '', 0),
        ('tianjin', 'flip', 0, .7, 1, '', 0),
        ('rush', 'soy_milk_cup', 2, 1.9, 1, 'coins', 0),
        ...
        ('journey', 'summary', 0, 6, 1, 'income', 0),
    ...
    assert abs(timeline - 60) < .001, timeline
    ...
    graph += '[1:a]atrim=duration=66,asetpts=PTS-STARTPTS,loudnorm=I=-21:LRA=9:TP=-3,afade=t=in:d=0.3,afade=t=out:st=63.5:d=2.5[music];'
    graph += '[0:a][music]amix=inputs=2:duration=first:normalize=0,loudnorm=I=-16:LRA=9:TP=-1.5[a]'
    final = OUT / '全世界等我开饭-Steam宣传片-中文-66秒.mp4'
    run(['-i', WORK / 'assembly.mp4', '-ss', 24, '-i', ROOT / 'resource/audio/demo/Carefree.mp3',
         '-filter_complex', graph, '-map', '[v]', '-map', '[a]', '-t', 66, '-r', 30,
    ...
    (OUT / 'edit-timeline-66.json').write_text(json.dumps(dict(duration=66, fps=30, cuts=manifest, captions=captions,
        end_card=dict(start=60, duration=6)), ensure_ascii=False, indent=2), encoding='utf-8')
    run(['-ss', 14.5, '-i', final, '-frames:v', 1, OUT / 'Steam视频封面-66秒.png'], 'poster.log')
    run(['-i', final, '-vf', 'fps=1/3,scale=480:270,tile=4x6', '-frames:v', 1, OUT / 'contact-sheet-66.jpg'], 'contact.log')
