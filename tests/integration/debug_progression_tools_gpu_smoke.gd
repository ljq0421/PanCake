...
func _run() -> void:
	...
	await _press_f3()
	_check(debug_panel.visible and not pancake_tier_one_button.disabled, "T1 checkpoint becomes clickable while the daily bill is open")
	await _click_control(pancake_tier_one_button)
	...
	_check(int(device_tiers.get("device.pancake_griddle", 0)) == 1 and main.workstation.daily_bill_panel.visible, "real pointer checkpoint reaches pancake T1 without closing the daily bill")
