...
public partial class XianVisualCapture : Node
{
    ...
    public override async void _Ready()
    {
        try
        {
            ...
            var portrait = _screen.Surfaces["customer0"].GetNode<CustomerPortraitView>("Portrait");
            Require(portrait.IsVisibleInTree() && portrait.GetChildren().OfType<TextureRect>().Count() == 2
                && portrait.GetChildren().OfType<Control>().All(layer => layer.MouseFilter == Control.MouseFilterEnum.Ignore),
                "顾客身体与头部分层显示且不拦截输入");
            var art = new TianjinArtCatalog();
            foreach (var state in new[] { ProjectCake.Customers.CustomerState.Happy, ProjectCake.Customers.CustomerState.Normal,
                ProjectCake.Customers.CustomerState.Impatient, ProjectCake.Customers.CustomerState.Angry })
            {
                customer!.State = state; _screen.Render();
                Require(portrait.HeadTexture == art.CustomerPortrait(customer.AppearanceId, state, false).Head,
                    $"顾客{state}切换共享表情");
            }
            ...
            Require(_screen.Workbench.GetNode<Control>("PauseMenu").Visible && controller.IsPaused
                && Find<Label>(_screen.Workbench.GetNode("PauseMenu"), l => l.Text.StartsWith("赠4个熟馍")).Visible,
                "打开当天教学仍保持暂停");
            await Shot("07-pause-help");
            ...
            controller.AbandonDay();
            foreach (int day in new[] { 1, 3, 6, 9, 12 })
            {
            ...
            Require(_screen.Initialize(catalog, save, controller, 1) && !_screen.SoupWorkbenchVisible && _screen.Session.Soup is null && _screen.Session.Oven is not null,
            ...
            var bubbles = _screen.Surfaces.Values.Where(v => v.Kind == "customer")
                .Select(v => v.GetNode<OrderBubbleView>("OrderBubble")).ToArray();
            Require(bubbles.All(b => b.IsVisibleInTree() && b.Size.X <= 332), "五位顾客显示独立订单气泡");
            Require(_screen.Surfaces.Values.Where(v => v.Kind == "customer")
                .SelectMany(v => v.GetChildren().OfType<Label>()).All(l => !l.Visible), "人物没有旧卡片标题和文字订单");
            for (int i = 0; i < bubbles.Length; i++)
            {
                Require(bubbles[i].GetGlobalRect().End.Y <= _screen.Surfaces[$"customer{i}"].GetNode<Control>("Portrait").GetGlobalRect().Position.Y,
                    $"气泡{i + 1}不遮挡人物");
                if (i > 0) Require(!bubbles[i - 1].GetGlobalRect().Intersects(bubbles[i].GetGlobalRect()), "相邻气泡不重叠");
            }
            ...
            controller.AbandonDay();
            Require(_screen.Initialize(catalog, save, controller, 7), "四配方素材场景初始化"); _screen.BeginDay(); Step(5);
            for (int variant = 0; variant < 4; variant++)
            {
                int portions = variant >= 2 ? 2 : 1; bool juiceOn = variant % 2 == 1;
                await Click(new(1145, 900)); await Gesture(new(1050, 900), new[] { new Vector2(1170, 900) });
                if (_screen.Session.Board.Portions < portions)
                    await Gesture(new(860, 650), Enumerable.Range(0, 10).Select(i => new Vector2(i % 2 == 0 ? 980 : 850, 650)).ToArray());
                for (int portion = 0; portion < portions; portion++) await Drag(new(830, 815), new(1110, 900));
                if (juiceOn) await Click(new(860, 920));
                Require(_screen.Session.Sandwich.MeatPortions == portions && _screen.Session.Sandwich.HasJuice == juiceOn,
                    $"素材配方{variant}实际制作状态正确");
                await Shot($"08-recipe-{variant}-open");
                await Click(new(1070, 1020)); await Shot($"08-recipe-{variant}-wrapped");
                Require(_screen.Session.Sandwich.TryTake(), "拍照后清理成品供下一配方验证");
            }
            controller.AbandonDay();
            ...
            Find<Button>(liveHub, b => b.Text.Contains("早餐地图")).EmitSignal(Button.SignalName.Pressed);
            globalSave.Data.UnlockedCityIds.Remove(StableIds.Cities.Xian);
            liveMap.Initialize(globalSave);
            await Frames(2);
            Button direct = Find<Button>(liveMap, b => b.Text == "测试直达西安");
            Require(direct.IsVisibleInTree(), "普通启动未解锁西安时测试按钮可见");
            await ClickGlobal(direct.GetGlobalRect().GetCenter());
            Require(liveHub.IsVisibleInTree() && !globalSave.Data.UnlockedCityIds.Contains(StableIds.Cities.Xian),
                "测试直达进入西安且不写入城市解锁");
