...
const OUTPUT_PATH := "res://tmp/validation/customer_20_neutral_v1_gpu_clear_1920x1080.png"
const EXPECTED_REGION := Rect2(503, 38, 526, 986)
...
func _run() -> void:
	...
	workstation.customer_queue.call("restore_active_customer", {}, &"customer_20")
	...
	# Formal service cards intentionally cover the legacy portrait area. Hide
	# them only for this art-review capture; gameplay scenes remain unchanged.
	...
	for _frame in 3:
		await process_frame
	print("CUSTOMER_20_PREVIEW_PORTRAIT visible=%s position=%s texture=%s" % [portrait.visible, portrait.global_position, portrait.texture.resource_path if portrait.texture != null else "<null>"])
	_check(portrait.texture != null and portrait.texture.resource_path.ends_with("customer_20_neutral_cropped.tres"), "customer_20 neutral portrait loads in the real workstation")
	if portrait.texture != null:
		var atlas := portrait.texture.get("atlas") as Texture2D
		_check(atlas != null and atlas.resource_path.ends_with("customer_20_neutral_v1_keyclean.png"), "AtlasTexture resolves the customer_20 neutral PNG")
		_check(portrait.texture.get("region") == EXPECTED_REGION, "customer_20 crop contains full hair, hands, and below-waist anchor")
...
func _finish() -> void:
	if _failures.is_empty():
		print("CUSTOMER_20_NEUTRAL_GPU_PREVIEW_PASS")
		quit(0)
		return
	printerr("CUSTOMER_20_NEUTRAL_GPU_PREVIEW_FAIL\n" + "\n".join(_failures))
	quit(1)
