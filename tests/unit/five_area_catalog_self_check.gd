...
func _run() -> void:
	...
	_check(_growth_price(&"growth.capacity.pancake_holding_tray.first_slot") == 40 and str(first_pancake_holding_tray.get("label", "")) == "煎饼暂存盘" and StringName(first_pancake_holding_tray.get("requires_area_id", &"")) == &"area.pancake" and StringName(first_pancake_holding_tray.get("kind", &"")) == &"storage", "the single four-serving pancake holding tray is one 40-coin storage upgrade")
