...
const OUTPUT_DIR := "res://tmp/validation/customer_12_states_v1_gpu"
const STATES := [&"neutral", &"impatient", &"satisfied", &"accepting_bag", &"paying_coins"]
const EXPECTED_FILES := {
	&"neutral": "customer_12_neutral_v1.png",
	&"impatient": "customer_12_impatient_v1.png",
	&"satisfied": "customer_12_satisfied_v1.png",
	&"accepting_bag": "customer_12_accepting_bag_v1.png",
	&"paying_coins": "customer_12_paying_coins_v1.png",
}
...
func _run() -> void:
	...
	for state in STATES:
		...
		for _frame in 3:
			await process_frame
		_check(portrait.texture != null and portrait.texture.resource_path.ends_with("customer_12_%s_cropped.tres" % state), "%s loads customer_12 AtlasTexture" % state)
		...
		var path := ProjectSettings.globalize_path("%s/customer_12_%s_v1_1920x1080.png" % [OUTPUT_DIR, state])
...
func _finish() -> void:
	if _failures.is_empty():
		print("CUSTOMER_12_STATES_GPU_PREVIEW_PASS")
		quit(0)
		return
	printerr("CUSTOMER_12_STATES_GPU_PREVIEW_FAIL\n" + "\n".join(_failures))
	quit(1)
