...
func _run() -> void:
	...
	_check(bool(delivered_youtiao.get("success", false)) and int(Dictionary(session.call("f3_machine_snapshot", &"device.youtiao_fryer")).get("quantity", 0)) == 1, "one ready youtiao is delivered directly from the raised basket while extra output remains in the fryer")
