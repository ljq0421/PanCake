...
func _run() -> void:
	...
	var second_pancake_holding_tray := CATALOG.growth_definition(&"growth.capacity.pancake_holding_tray.second_slot")
	_check(_growth_price(&"growth.capacity.pancake_holding_tray.first_slot") == 40 and StringName(first_pancake_holding_tray.get("requires_area_id", &"")) == &"area.pancake" and StringName(first_pancake_holding_tray.get("kind", &"")) == &"storage", "first pancake holding tray is a 40-coin pancake storage upgrade")
	_check(_growth_price(&"growth.capacity.pancake_holding_tray.second_slot") == 60 and _requires(&"growth.capacity.pancake_holding_tray.second_slot", &"growth.capacity.pancake_holding_tray.first_slot"), "second pancake holding tray costs 60 and requires the first tray")
