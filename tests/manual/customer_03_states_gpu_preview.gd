...
const OUTPUT_DIR := "res://tmp/validation/customer_03_states_v4_colorlocked_gpu"
const STATES := [&"neutral", &"impatient", &"satisfied", &"accepting_bag", &"paying_coins"]
const EXPECTED_FILES := {
	&"neutral": "customer_03_neutral_v4_chinese.png",
	&"impatient": "customer_03_impatient_v4_colorlocked.png",
	&"satisfied": "customer_03_satisfied_v4_colorlocked.png",
	&"accepting_bag": "customer_03_accepting_bag_v4_colorlocked.png",
	&"paying_coins": "customer_03_paying_coins_v4_colorlocked.png",
}
...
func _run() -> void:
	...
	for state in STATES:
		...
		if portrait.texture != null:
			...
			_check(atlas != null and atlas.resource_path.ends_with(String(EXPECTED_FILES[state])), "%s state resolves its v4 texture" % state)
		await RenderingServer.frame_post_draw
		var path := ProjectSettings.globalize_path("%s/customer_03_%s_v4_colorlocked_1920x1080.png" % [OUTPUT_DIR, state])
