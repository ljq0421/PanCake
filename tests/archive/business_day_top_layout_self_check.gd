...
func _run() -> void:
	...
	var attention_rail := workstation.get_node("SafeArea/AttentionRail") as Control
	...
	_check(not summary_card.visible or not _rects_overlap(timer_background.get_global_rect(), summary_card.get_global_rect()), "倒计时与可见的本单结算使用独立区域")
	_check(not _rects_overlap(compact_status.get_global_rect(), top_warning.get_global_rect()), "设备内操作状态与顶部瞬时警告使用独立区域")
	_check(top_warning.get_global_rect() == attention_rail.get_global_rect(), "瞬时警告复用顶部提醒入口区域")
	_check(compact_status.get_global_rect().position.y >= 646.0, "操作状态回到工作台上下文，不再占用顶部 HUD")
